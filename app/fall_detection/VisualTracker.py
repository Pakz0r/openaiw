...
class Tracker:
    ...
    def _manage_windows(self):
        ...
        '''
        ready_windows = []
        unprocessed_ready_windows = []
        for i, w in enumerate(self.windows):
            if(len(w)==75):
                ready_windows.append(postprocess(w))
                unprocessed_ready_windows.append(np.reshape(w, (75,38))) #to delete every line of code linked to unprocessed_ready_windows
                del self.windows[i][:50]
        return ready_windows, unprocessed_ready_windows

    def _draw_windows(self, windows):
        # PRINT DELLE WINDOWS
        for _, window in enumerate(windows):
            for frame_index, skeleton in enumerate(window):
                draw(skeleton, self.global_counter, frame_index, "output_windows")
            generate_video( self.global_counter, "output_windows")
    ...
    def run(self, keypoints):
        ...
        self._track()
        windows, unprocessed_windows = self._manage_windows()
        return windows, unprocessed_windows, self.previous_skeletons.copy()